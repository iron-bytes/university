...
Si può dimostrare che è possibile schedulare il trasferimento del file in modo tale che 
...
Una socket UDP viene identificata completamente da una coppia che consiste di un indirizzo IP e un numero di porta.
...
Le socket in cui solo due elementi sono noti: IP e numero di porta sono adibite a ricevere richieste di connessione. Ad esempio la porta \texttt{(IndirizzoIp, 80, null, null)} serve solo a ricevere richieste di instaurazione di connessione sulla porta 80.
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|}
		\hline
		\textbf{IP Origine} & \textbf{Porta Origine} & \textbf{IP Destinazione} & \textbf{Porta Destinazione} \\
		\hline
...
\subsubsection*{Trasferimento dati affidabile su un canale con perdite ed errori sui bit: RDR 3.0}
...
\subsubsection*{Recap di RDT}
\textbf{RDT 1.0:} Il canale sottostante è perfettamente affidabile
\\
\textbf{RDT 2.0:} Il canale sottostante non è affidabile quindi introduciamo ACK e NAK. Il problema di RDT 2.0 è che anche gli ACK e i NAK possono essere corrotti
\\
\textbf{RDT 2.1:} Per ovviare ai problemi di RDT 2.0 facciamo in modo che il mittente ritrasmetta l'ultimo pacchetto quando riceve un ACK o un NAK corrotto.
\\
Notiamo però quanto segue:
\begin{itemize}
	\item Il destinatario manda un ACK che arriva corrotto al mittente. A questo punto il destinatario si aspetta il segmento (n+1)-esimo ma riceverà il segmento n-esimo
	\item Il destinatario manda un NAK che non arriva al mittente. A questo punto il destinatario si aspetta il segmento n-esimo ma riceve il segmento (n+1)-esimo
\end{itemize}
Per ovviare a ciò usiamo i numeri di sequenza
\\
\textbf{RDT 3.0:} Se i pacchetti vanno persi abbiamo bisogno di un timer
...
In altre parole, il mittente SR manda avanti la finestra quando riceve un ACK per \texttt{base}. Il ricevente SR manda avanti la finestra quando riceve il pacchetto che si aspetta di ricevere corretto e in ordine.
...
In TCP non si usa un NAK esplicito perché un ACK duplicato è implicitamente un NAK.
...
\begin{enumerate}
	\item Dati provenienti dall'applicazione: TCP li incapsula in uno o più segmenti che vengono successivamente passati a IP. Inoltre, avvia il timer.
...
\begin{enumerate}
	...
	\item Arrivo di segmento che colma parzialmente o completamente un buco: Invia un ACK relativo a questo segmento solo se colma l'estremo sinistro di un buco, altrimenti invia un ACK duplicato relativo all'ultimo byte ricevuto in ordine.
\end{enumerate}
...
Cresce esponenzialmente a ogni RTT perché dopo la ricezione di un ACK del primo segmento trasmesso potrà trasmetterne due, dopodiché potrà trasmetterne 4,8,16 e così via. Ciò viene fatto per individuare velocemente la capacità della rete. In altre parole raddoppia a ogni RTT.
...
Quando siamo in Fast Recovery vogliamo semplicemente ricevere l'ACK del segmento inviato con la ritrasmissione rapida senza ridurre drasticamente la velocità di trasmissione.
...
\begin{enumerate}
	...
	\item Cifra $H(m)$ con la propria chiave privata: $ K^{-}_{A}(H(m))$
	\item Crea un messaggio $(m, K^{-}_{A}(H(m)))$ e lo invia a Bob
	\item Quando Bob riceve il messaggio, calcola $H(m)$ e lo confronta con $K^{+}_{A}(K^{-}_{A}(H(m)))$. Se coincide, allora ha autenticato Alice.
\end{enumerate}
...
Se in un firewall tradizionale le decisioni sono basate su ogni pacchetto, in modo indipendente l'uno dall'altro, i firewall con informazioni di stato tracciano le connessioni TCP e usano questa conoscenza per effettuare decisioni.
