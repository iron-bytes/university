...
\subsection*{5.3 Sistemi autonomi e OSPF}
...
\subsection*{5.4 Instradamento inter-AS: BGP}
...
\subsubsection*{5.4.1 Il ruolo di BGP}
...
\begin{itemize}
	...
	\item Determinare il percorso migliore verso una sottorete
\end{itemize}

\subsubsection*{5.4.2 Distribuzione delle informazioni dei cammini in BGP}
...
Come viene comunicata l'esistenza di una sottorete verso tutti i sistemi autonomi? In modo molto semplice: una sottorete si annuncia a un AS il quale provvede a comunicarlo a tutti gli AS con cui confina, ripetendo fintantoché tutti i sistemi autonomi non hanno registrato la nuova sottorete. Un AS comunica con altri AS tramite i router di bordo: possono esserci anche più router di bordo in uno stesso Sistema Autonomo non direttamente connessi; in tal caso servirà mandare internamente all'AS dei messaggi BGP (iBGP). I messaggi BGP scambiati tra due router gateway di due AS diversi prendono il nome di messaggi eBGP (external). \textbf{SCRIVERE MEGLIO NON MI PIACE, TROPPO CONFUSO}
