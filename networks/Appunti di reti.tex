...
Se il collegamento fisico è condiviso tra più di due nodi ci troviamo dinnanzi un canale broadcast (es. onde elettromagnetiche). D'inverso, se per un dato collegamento ci sono solo due nodi allora siamo in presenza di un collegamento punto-punto.
...
\textbf{CAPIRE MEGLIO} --- DA SCRIVERE!!

\section*{6.3 Collegamenti broadcast e protocolli di accesso multiplo}
Il collegamento punto a punto è costituito da un trasmittente posto a un capo del collegamento e da un unico ricevente all'altra.s
...
\begin{enumerate}
	...
	\item Quando $M$ nodi devono inviare dati, questi dispongono di un throughput pari a $R/M$ bit al secondo
...
Il valore $p*$ che sostituito a $p$ massimissi l'espressione è $0,37$. Pertanto, dato che abbiamo il valore $p$ che massimizza l'espressione, impostiamo $0,37$ essere tale probabilità. Ciò però implica che i nodi trasmetteranno informazione utile solo il $37\%$ delle volte e di conseguenza la banda effettivamente utilizzata sarà del $37\%$

\subsubsection*{ALOHA}
Slotted ALOHA richiede che tutti i nodi sincronizzino le loro trasmissioni mentre ALOHA è completamete decentralizzato.
...
Ogni nodo ha un modulo ARP che riceve in input un indirizzo IP e restituisce un indirizzo MAC. ARP risolve soltanto gli indirizzi IP per i nodi della stessa sottorete. È in grado di fare ciò per mezzo di una tabella ARP che contiene la corrispondenza degli indirizzi più un valore relativo al TTL che indica quando bisogna eliminare una voce dalla tabella.
